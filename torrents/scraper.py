...
if category=='tv':
    ...
    new_title = new_soup.find('div',class_='title_wrapper').find('h1').get_text().strip()
    print new_title
    ...
    for item in episodes_list:
        dictionary = {"category":category,"title":new_title,"item":item}
        json.dump(dictionary,f)
        f.write("\n")
else:
    dictionary = {"category":category,"title":title}
    json.dump(dictionary,f)
    f.write("\n")
f.close()
